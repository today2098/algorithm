...
data:
  ...
  bundledCode: "#line 1 \"test/aoj-DSL_2_I.test.cpp\"\n#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/courses/library/3/DSL/all/DSL_2_I\"\
    ...
    \u7B97\u95A2\u6570). S\xD7S\u2192S\u3092\u8A08\u7B97\u3059\u308B\uFF0E\n    Mapping\
    \ m_mapping;          // S m_mapping(F f,S x):=(\u5199\u50CF). f(x)\u3092\u8FD4\
    ...
    \ q;\n    std::cin >> n >> q;\n\n    using S = struct {\n        int val;\n  \
    \      int size;\n    };\n    using F = int;\n    constexpr S e = (S){0, 0};\n\
    \    constexpr F id = 1e9;\n    auto op = [&](const S &lhs, const S &rhs) -> S\
    \ { return (S){lhs.val + rhs.val, lhs.size + rhs.size}; };\n    auto mapping =\
    \ [&](const F &f, const S &x) -> S { return (S){(f == id ? x.val : f * x.size),\
