...
public:
    ...
    std::vector<std::vector<int> > scc(const std::pair<int, std::vector<int> > &p) const {
        return scc(p.first, p.second);
    }
    ...
    std::vector<std::vector<int> > directed_acyclic_graph(const std::pair<int, std::vector<int> > &p) const {
