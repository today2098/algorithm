...
class HLD {
    ...
    std::vector<int> m_vid;              // m_vid[v]:=(ノードvの行きかけ順序).
    ...
    int dfs1(int u, int p) {
        ...
        for(int &v : m_g[u]) {
            if(v == p) continue;
            m_sub[u] += dfs1(v, u);
            if(m_sub[v] > m_sub[m_g[u][0]]) std::swap(v, m_g[u][0]);  // m_g[u][0]にheavy childを格納する．
        }
        return m_sub[u];
    }
...
public:
    HLD() : HLD(0) {}
    explicit HLD(size_t vn) : m_vn(vn), m_g(vn), m_par(vn), m_sub(vn), m_head(vn), m_vid(vn, -1) {}
    ...
    std::vector<std::pair<int, int> > vertex_query_range(int u, int v) const {
        assert(0 <= u and u < order());
        assert(0 <= v and v < order());
        ...
        while(true) {
            ...
            res.emplace_back(m_vid[m_head[v]], m_vid[v] + 1);
            v = m_par[m_head[v]];
        }
        return res;
    }
    ...
    std::pair<int, int> subtree_vertex_query_range(int v) const {
        ...
        return {m_vid[v], m_vid[v] + m_sub[v]};
    }
    // 木上のノードuとvの最も近い共通の祖先を求める．O(log|V|).
    int lca(int u, int v) const {
        assert(0 <= u and u < order());
        assert(0 <= v and v < order());
        if(m_vid[u] == -1 or m_vid[v] == -1) return -1;
        while(true) {
            if(m_vid[u] > m_vid[v]) std::swap(u, v);
