...
class LCA {
    ...
    void dfs(int u, int p, int d, int &now) {
        assert(m_ord[u] == -1);
        m_ord[u] = now++;
        m_par[u][0] = p;
        m_depth[u] = d;
        for(int v : m_g[u]) {
            if(v != p) dfs(v, u, d + 1, now);
        }
    }

public:
    LCA() : LCA(0) {}
    explicit LCA(size_t vn) : m_vn(vn), m_l(0), m_g(vn), m_depth(vn, -1), m_ord(vn, -1) {
    ...
    int lca(int u, int v) const {
        ...
        v = up(v, depth(v) - depth(u));  // 同じ深さに合わせる．
        if(u == v) return u;
    ...
    std::pair<int, std::vector<std::vector<int> > > auxiliary_tree(std::vector<int> &vs) const {
        ...
        for(int i = 1; i < n; ++i) {
            ...
            if(w != vs[i - 1]) {
                int v = st.top();
                st.pop();
                while(!st.empty() and depth(st.top()) > depth(w)) {
