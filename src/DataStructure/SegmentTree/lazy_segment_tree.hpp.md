...
data:
  ...
  bundledCode: "#line 1 \"src/DataStructure/SegmentTree/lazy_segment_tree.hpp\"\n\n\
    ...
    \u9805\u6F14\u7B97\u95A2\u6570). S\xD7S\u2192S\u3092\u8A08\u7B97\u3059\u308B\uFF0E\
    \n    Mapping m_mapping;          // S m_mapping(F f,S x):=(\u5199\u50CF). f(x)\u3092\
    \u8FD4\u3059\uFF0E\n    Composition m_composition;  // F m_composition(F f,F g):=(\u5199\
    \u50CF\u306E\u5408\u6210). f\u2218g\u3092\u8FD4\u3059\uFF0E\n    S m_e;      \
    \                // m_e:=(\u5358\u4F4D\u5143).\n    F m_id;                  \
    \   // m_id:=(\u6052\u7B49\u5199\u50CF).\n    int m_sz;                   // m_sz:=(\u8981\
    \u7D20\u6570).\n    int m_n;                    // m_n:=(\u4E8C\u5206\u6728\u306E\
    \u8449\u6570).\n    int m_depth;                // m_depth:=(\u4E8C\u5206\u6728\
...
通常の Segment Tree が行うクエリは「一点更新・区間取得」であるのに対し，Lazy Segment Tree（遅延評価セグメント木）は「**区間更新**・区間取得」である．

具体的には，長さ $N$ の配列 $\{a_0, a_1, \ldots, a_{n-1}\}$ に対して，次のクエリを $\mathcal{O}(\log N)$ で行う．
...
本ライブラリの実装では [Segment Tree](https://today2098.github.io/algorithm/src/DataStructure/SegmentTree/segment_tree.hpp) と同様に抽象化を行っており，オブジェクト定義時にモノイド $(S, \bullet : S \times S \rightarrow S, e \in S)$ と，以下の条件を満たす写像の集合 $F$ を定義する．

- $\forall x \in S$ に対して $\operatorname{id}(x)=x$ となる恒等写像 $\operatorname{id}$ を含む．
- $\forall f, \forall g \in F$ に対して $f \circ g \in F$ である（写像の合成について閉じている）．
- $\forall f \in F, \forall x, \forall y \in S$ に対して $f(x \bullet y) = f(x) \bullet f(y)$ である． 


## 参考文献

1. atcoder. "Lazy Segtree". AC Library. <https://atcoder.github.io/ac-library/production/document_ja/lazysegtree.html>.
