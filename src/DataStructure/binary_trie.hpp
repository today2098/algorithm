...
private:
    ...
    void dfs_print(std::ostream &os, Node *p, std::bitset<B> &x, int shift = B - 1) const {
        if(shift < 0) {
            os << "  {0b" << x;
        ...
        for(bool bit : {0, 1}) {
            x[shift] = bit;
            bit ^= m_bias[shift];
            if(p->ch[bit]) dfs_print(os, p->ch[bit], x, shift - 1);
        }
    }
...
public:
    ...
    // 全要素値にxorの操作を行う．O(B).
    void operate_xor_all(const std::bitset<B> &x) { m_bias ^= x; }
    // 全要素を削除する．
    ...
    friend std::ostream &operator<<(std::ostream &os, const BinaryTrie &ob) {
        std::bitset<B> x(0);
        os << "[\n";
        if(ob.m_root) ob.dfs_print(os, ob.m_root, x);
        os << "]";
        return os;
    }
};
