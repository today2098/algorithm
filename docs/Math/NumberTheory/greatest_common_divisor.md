...
本ライブラリでは「ユークリッドの互除法 (Euclidean algorithm)」を用いて実装しており，計算量は $\mathcal{O}(\log(\min(a,b)))$ となる．
...
先の式 $r_i - \lfloor r_i / r_{i+1} \rfloor \cdot r_{i+1} = r_{i+2}$ から，$r_{i+2} < r_i / 2$ である．
...
| 関数                                                                                                     | 説明                                           | 計算量                         |
| -------------------------------------------------------------------------------------------------------- | ---------------------------------------------- | ------------------------------ |
| `template <std::integral T, std::integral U>`<br>`constexpr auto gcd(T a, U b)`                          | $\|a\|, \|b\|$ の最大公約数を求める．          | $\mathcal{O}(\log(\min(a,b)))$ |
| `template <std::integral Type, std::integral... Args>`<br>`constexpr auto gcd(Type first, Args... args)` | 複数の整数の絶対値に対する最大公約数を求める． | -                              |
